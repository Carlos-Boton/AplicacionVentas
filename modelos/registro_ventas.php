...
public function listar(){
	$sql="SELECT v.idventa,DATE(v.fecha_hora) as fecha,v.idcliente,p.nombre as cliente, v.comprobante,v.total_venta,v.estado FROM ventas v INNER JOIN clientes p ON v.idcliente=p.idclientes ORDER BY v.idventa DESC";
	return ejecutarConsulta($sql);
}

public function listarFecha($fecha_hora){
	$sql="SELECT v.idventa, DATE(v.fecha_hora) as fecha, v.idcliente, p.nombre as cliente, v.comprobante, v.total_venta, v.estado FROM ventas v INNER JOIN clientes p ON v.idcliente = p.idclientes WHERE DATE(v.fecha_hora) = $fecha_hora";
	return ejecutarConsulta($sql);
}
