...
switch ($_GET["op"]) {
	case 'guardaryeditar':
	if (empty($idventa)) {
		$rspta=$venta->insertar($idclientes,$comprobante,$fecha_hora,$total_venta,$_POST["idarticulo"],$_POST["cantidad"],$_POST["descripcion"]); 
    ...
    case 'listar':
        ...
        case 'listarArticulos':
            ...
            while ($reg = $rspta->fetch_object()) {
                $data[] = array(
                    "0" => '<button class="btn btn-warning" onclick="agregarDetalle('.$reg->idarticulo.', \''.$reg->nombre.'\', \''.$reg->descripcion.'\', this)"><i class="bi bi-plus-lg"></i></button>',
                    "1" => '
                        <div class="input-group">
                            <button class="btn btn-primary" type="button" onclick="decreaseValue(this)">-</button>
                            <input type="text" id="numberInput" class="numberInput form-control p-1" value="1" min="0">
                            <button class="btn btn-primary" type="button" onclick="increaseValue(this)">+</button>
                        </div>
                    ',
                    "2" => $reg->nombre,
                    "3" => $reg->descripcion,
